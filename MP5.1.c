...
__global__ void total(float *input, float *output, int inputLen, int outputLen) {
  ...
  int t = threadIdx.x;
  int startIdx = blockIdx.x * blockDim.x * 2;
  
  if(startIdx + t < inputLen) 
    SM[t] = input[startIdx + t];
  else 
    SM[t] = 0;
  
  if(startIdx + t + BLOCK_SIZE < inputLen)
    SM[t + BLOCK_SIZE] = input[startIdx + t + BLOCK_SIZE];
  else
    SM[t + BLOCK_SIZE] = 0;
