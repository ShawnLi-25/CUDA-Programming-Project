...
__global__ void spmvJDSKernel(float *out, int *matColStart, int *matCols,
  ...
  if(row < dim) {
    ...
    //matRows is the length of each original row; when sec == rowLen, we've dnoe with all the elements for this row(no need to further iterate) 
...
static void spmvJDS(float *out, int *matColStart, int *matCols,
  ...
  /*
    ...
    Here, dim is the number of row
    Each thread takes care of one row
  */
  dim3 dimGrid = dim3(ceil(1.0 * dim/ TILE_WIDTH), 1, 1);
  dim3 dimBlock = dim3(TILE_WIDTH, 1, 1);
...
int main(int argc, char **argv) {
  ...
  maxRowNNZ = hostJDSRows[0]; //Represent max row length of original matrix

  printf("The Number of data is: %d, The length of dim is: %d\n, The length of col is: %d, The length of row is: %d\n", ndata, dim, ncols, nrows);

  wbTime_start(GPU, "Allocating GPU memory.");
  cudaMalloc((void **)&deviceJDSColStart, sizeof(int) * maxRowNNZ); // Column Pointer(start position for each iteration) for Transposition
  cudaMalloc((void **)&deviceJDSCols, sizeof(int) * ndata);         // Column Index
  cudaMalloc((void **)&deviceJDSRowPerm, sizeof(int) * dim);        // Original Row Number
  cudaMalloc((void **)&deviceJDSRows, sizeof(int) * dim);           // Length of each row
  cudaMalloc((void **)&deviceJDSData, sizeof(float) * ndata);       // Non-zero Data
